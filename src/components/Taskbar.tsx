...
import { faFileCirclePlus, faFileCircleCheck, faFileCircleQuestion, faFileCircleMinus } from '@fortawesome/free-solid-svg-icons'

const Buttons = [
    { name: "add",       link: "/home",      icon: faFileCirclePlus },
    { name: "edit",         link: "/projects",  icon: faFileCircleQuestion },
    { name: "rmv",       link: "/contact",   icon: faFileCircleMinus },
    { name: "do",     link: "/about",     icon: faFileCircleCheck }
]
...
export default function Taskbar(props: any) {
    return(
        <section id={"taskbar"} className={"taskbar"}>
            ...
            <svg viewBox="0 0 185 100" fill="none" xmlns="http://www.w3.org/2000/svg" className={"m-auto"}>
                <rect y="10" width="173" height="79" fill="#A04732" fill-opacity="0.5"/>
                <rect x="12" y="21" width="173" height="79" fill="#A04732" fill-opacity="0.5"/>
            ...
            <div id={"taskbar-navigation"} className={"taskbar-container"}>
                {Buttons.map((button, index) => { return (<a key={index} href={button.link} className={"taskbar-button group"}><FontAwesomeIcon icon={button.icon} size={"lg"} /><p>{button.name}</p></a>)})}
            </div>
        </section>
    )
}